...
export const PERMISSION_CATALOG: readonly PermissionSpec[] = [
  ...
  },
  {
    ...
    group: "data",
    title: "读取附加文件",
    desc: "读取你粘贴/拖入搜索框的文件与文件夹内容（仅限已附加的路径，无法访问其它本地文件）",
    risk: "high",
    runtime: "both",
    revokeImpact: "插件将无法再读取搜索框里附加的文件/文件夹",
  },
  {
    id: "env.read",
    group: "data",
    title: "使用环境变量",
    desc: "使用你在「设置 → 环境变量」里集中维护的变量，用于代替在插件里另存一份密钥。逐项授权：未授权的变量对该插件既不可见、也不会注入它的后台进程",
    risk: "high",
    scoped: true,
    runtime: "both",
